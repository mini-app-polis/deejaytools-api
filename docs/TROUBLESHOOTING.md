...
**Symptom:** API process exits immediately on boot; stack trace points at `src/db/index.ts`.

**Cause:** `DATABASE_URL` is missing or empty. The DB module throws before the server listens.

**Fix:**

1. Set `DATABASE_URL` to a valid Postgres connection string (`.env` locally, Railway variables in production).
...
**Symptom:** Blank page; browser console shows uncaught error at startup from `deejaytools-com/src/main.tsx`.
...
3. Verify locally: copy `deejaytools-com/.env.example` to `.env.local` and set the key before `pnpm dev` in deejaytools-com.
...
3. Tune pool settings in Railway (all optional, defaults in `src/db/index.ts`):
...
**Cause:** Another process (often a previous `pnpm dev`) is bound to `3001`.

**Fix:**

1. Find the process: `lsof -i :3001` (macOS/Linux).
2. Stop it: `kill <PID>` or stop the other terminal session.
3. Or set a different `PORT` in `.env` and point `VITE_API_URL` / Vite proxy at the new port.
...
1. Set all three on Railway (see `.env.example`).
2. Redeploy.
3. Retry upload.
...
2. Code applies `.replace(/\\n/g, "\n")` in `src/services/drive.ts` — the stored value must use **literal backslash-n**, not actual newlines, if your platform requires single-line secrets.
...
**Cause:** Server magic-byte sniffing (`detectAudioFormat` in `src/services/audioFormat.ts`) did not recognize the file — wrong type, corrupt file, or renamed non-audio extension. Client MIME type is intentionally not trusted (iOS often sends `application/octet-stream`).
...
**Symptom:** Mid-upload failure in the browser; toast or inline error with this exact string from `deejaytools-com/src/lib/chunkedSongUpload.ts`.
...
2. If uploads fail consistently with no status code, verify the Fastly body-drain server in `src/index.ts` is still deployed (see [`DEPLOYMENT.md`](./DEPLOYMENT.md)).
...
**Cause:** Frontend origin not listed in Railway `CORS_ORIGINS`. Default when unset is only `http://localhost:5173` (`src/app.ts`).
...
**Cause:** `TICK_SECRET` is **unset** (`undefined`). Code only gates when `secret !== undefined` (`src/app.ts`).
...
**Cause:** Fastly idle write-timeout before server consumed request body (see `src/index.ts` comment). Regression if custom `createServer` body drain is removed.
...
- [`docs/AUTHENTICATION.md`](./AUTHENTICATION.md) — auth codes and guards
- [`deejaytools-com` docs/ARCHITECTURE.md](https://github.com/mini-app-polis/deejaytools-com/blob/main/docs/ARCHITECTURE.md) — frontend upload and auth sync behaviour
