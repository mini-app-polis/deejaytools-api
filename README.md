...
West Coast Swing routine and floor-trial management platform. The web
app lives in [`deejaytools-com`](https://github.com/mini-app-polis/deejaytools-com/blob/main/).

Contract schemas live in [`src/schemas`](src/schemas/index.ts). The web
app keeps its own copy; both must change together when the contract does.
...
pnpm dev       # start the API in watch mode (http://localhost:3001)
pnpm test      # run the vitest suite
pnpm build     # produce the production build
```

Copy `.env.example` to `.env` and fill in the required variables listed under Environment variables.

## Migrations

Managed by Drizzle ORM. Migration files live under `drizzle/`.

- `pnpm db:generate` — author a new migration from schema
  changes.
